...
#if defined(NDEBUG)
# error "Eurocash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x10280fe5c2ab97727321bef7f6bd7eeb1ba2c874c028adf4da855d59f76b7818");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Eurocash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Eurocash Signed Message:\n";
...
{
    // Eurocash: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 5000); // Eurocash: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Eurocash: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Eurocash: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Eurocash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Eurocash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe6e67172ffd0b407c12c7b7a65888edbda35092d954bf21fd0eb4d6dff211c6d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04538d7710cd5e8d03ec7641eb9be5d4da4e0739867bcf4fe07c203e405629702bcef5135f0aa80cc305fa878d2317ab7163fe5a0cf18300fb74050db287dccdae") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1558821624;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085947139;

        if (fTestNet)
        {
            block.nTime    = 1558821599;
            block.nNonce   = 386279953;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf8f2eed9c7b81cd0474e804f9f3d69feb89da1d8ff86cbd86de4e913831b15c9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb3, 0xdb }; // Eurocash: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EurocashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EurocashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EurocashMiner(CWallet *pwallet)
{
    printf("EurocashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("eurocash-miner");
    ...
    try { loop {
        ...
        printf("Running EurocashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EurocashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EurocashMiner, pwallet));
}
